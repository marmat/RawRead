...
class Device:
    ...
    def __init__(self, path, force=False, permissions="rb"):
        """Class constructor.
        ...
        self.valid_nofs = (self._device_handle.read(len(NOFS_SIGNATURE)) ==
                           NOFS_SIGNATURE)
    ...
    def get_contents(self):
        ...
        * EOF is reached
        ...
        # If we have a valid NoFS, ignore the first few bytes so that they
        # don't show up in the returned content.  Furthermore, set up which
        # symbols will end the content getting.
        terminal = chr(0x00)
        if self.valid_nofs:
    ...
    def erase(self, complete=False):
        ...
        # If only a partial erase is wished and the count of sectors hasn't
        # been determined, sweep through the contents.
        if self.sectors_read is None and not complete:
            self.get_contents()
    ...
    def initialize_nofs(self):
        ...
        """
        if self.valid_nofs:
            print "ERROR: the device already contains a valid NoFS. \
        ...
        # Erase the first few sectors and write header. Please note: while
        # erasing, a NOFS_TERMINAL will be placed inside every first byte of
        # an erased sector.
    ...
    def _write_header(self):
        ...
        first_sector += (NOFS_SECTOR_SIZE - len(NOFS_HEADER) -
                         len(NOFS_TERMINAL)) * chr(0xFF)
...
def get_possible_devices():
    ...
    if sys.platform.startswith("win32"):
        for i in range(MAX_DEVICES):
            possible_devices.append("\\\\.\\PhysicalDrive%d" % i)
...
def get_removable_devices(opts):
    """Method which is used to determine devices that might be
    removable drives.
    """
...
def get_nofs_device():
    """Returns the first device found which contains a valid NoFS"""
...
def main():
    ...
    parser.add_option("-E", "--full-erase", action="store_true",
                      dest="full_erase", default=False, help="disk will be "
                      "completely overwritten. May take a long time depending "
                      "on the size of the device specified by INPUT. will "
                      "only be performed if device contains valid NoFS or "
                      "-f is set. If used in combination with -o, the device "
                      "will be read out first and erased afterwards.")
    ...
    if options.create_disk and options.output_file:
        parser.error("can't use -c and specify an output file at "
                     "the same time!")
    ...
    if options.create_disk and options.input_device and not options.force:
        parser.error("can't use -c and -i unless -f is set (are you "
    ...
    if options.create_disk:
        ...
        if not devices:
            ...
            print "\nWARNING: More than one suitable device has been detected.\
                  \nNo action will be performed. Please run the program again,\
                  \nusing the -i parameter to select one of the following\
                  \ndevices and -f to enforce your selection (dangerous!):\n"
            print devices
        else:
    ...
    if options.input_device:
        ...
        if device is None or not device.valid_nofs:
            print "ERROR: No valid NoFS device found"
            sys.exit(0)

    # 2) Read from the input device
    ...
    # 4) Check if it should be erased
    ...
    # 5) Close the output stream (if it's not stdout)
    if options.output_file:
        output_handle.close()
